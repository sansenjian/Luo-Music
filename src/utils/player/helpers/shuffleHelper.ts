...
export class ShuffleHelper {
  ...
  getNextShuffledIndex(shuffledIndices: number[], currentShuffleIndex: number): number {
  ...
  getPrevShuffledIndex(shuffledIndices: number[], currentShuffleIndex: number): number {
    if (!shuffledIndices || shuffledIndices.length === 0) return -1
