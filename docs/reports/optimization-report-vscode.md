...
manualChunks: id => {
  ...
  // UI 相关
  if (id.includes('@fontsource/') || id.includes('animejs')) return 'vendor-ui'

  // 数据相关
  if (id.includes('axios') || id.includes('@tanstack/vue-query')) return 'vendor-data'

  // 工具库
  if (id.includes('date-fns') || id.includes('zod')) return 'vendor-utils'

  // 大组件单独打包
  if (id.includes('components/UserCenter.vue')) return 'component-user-center'
}
```
