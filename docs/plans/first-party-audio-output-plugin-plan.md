...
- 具体格式边界见 [原生音频输出格式支持](../reference/native-audio-output-format-support.md)；本地路由按扩展名白名单进入 native，在线路由会尝试缓存后交给 helper，实际仍以解码结果为准。
...
- 音频输出协议已加入 DSP 设置草案：`dsp.enabled`、`dsp.headroomDb` 和最多 12 段 EQ band。插件设置页当前只暴露 DSP 开关和 headroom；开启“强制 bit-perfect 候选输出”时会自动隐藏并关闭 DSP，避免把 bit-perfect 候选路径变成带处理链路的输出。
...
- DSP 当前只完成共享协议、设置快照和 bit-perfect 互斥约束；`audio-engine-dsp` 仍是 Phase 1 crate，尚未插入 Rust PCM 输出链路，也没有完整 EQ 编辑器。
