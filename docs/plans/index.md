...
| 优先级 | 文档                                                                 | 当前状态                                                           | 下一步                                                                            |
| ------ | -------------------------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| P0     | [当前项目规划](/plans/current-project-plan)                          | 汇总本地音乐库、插件平台、Rust 原生音频和文档治理的下一阶段执行面  | 先收尾当前功能分支，再按 P1 稳定本地库、插件平台和 native audio                   |
...
| P1     | [插件体系拓展架构](/architecture/plugin-extension-architecture)      | SDK、外部插件宿主、`ctx.secrets`、第一方 SMTC / cover-swipe 已落地 | 补 runtime hook 执行器、extension registry、桌面歌词 / 波形插件化                 |
| P1     | [插件规范](/reference/plugin-specification)                          | 当前插件 API 和 manifest 规范                                      | 随 SDK 能力变化同步更新                                                           |
