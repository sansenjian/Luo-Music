...
import type { INVOKE_CHANNELS, SEND_CHANNELS, RECEIVE_CHANNELS } from './channels'
...
export interface InvokeChannelMap {
  ...
  [INVOKE_CHANNELS.SERVICE_STATUS_ALL]: {
    params: []
    result: Record<string, 'running' | 'stopped' | 'error'>
  }
...
export interface SendChannelMap {
  ...
  [SEND_CHANNELS.LOG_MESSAGE]: {
    params: [{ level: string; module: string; message: string; data?: unknown }]
  }
  [SEND_CHANNELS.ERROR_REPORT]: {
    params: [{ code: string; message: string; stack?: string; data?: unknown }]
  }
}
...
export interface ReceiveChannelMap {
  ...
  [RECEIVE_CHANNELS.LYRIC_TIME_UPDATE]: {
    payload: { time: number; index: number; text: string; trans: string; roma: string }
  }
