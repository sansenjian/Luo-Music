...
| 文件 | 修复内容 |
|------|---------|
| `electron/sandbox/index.ts` | 修复 10+ 处中文注释乱码（如"鏍稿績"→"核心"、"鏈嶅姟"→"服务"等） |
| `electron/ipc/index.ts` | 修复"涓棿浠？→"中间件"、"澶勭悊鍣？→"处理器"等 |
| `electron/ipc/types.ts` | 全面修复文件头、章节标记、注释中的乱码（如"閫氶亾绫诲瀷"→"通道类型"） |
