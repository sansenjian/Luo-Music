...
- 需要持久化账号态、Cookie 或用户配置的插件应声明 `permissions.storage: true`，敏感凭据应使用 `secrets`。
    ...
    "themeResources": [
      {
        ...
        "cssText": ":root[data-render-style='example.ocean'] [data-ui='player'] { border-radius: 24px; }"
      }
    ],
    "settings": [
      {
...
为避免第三方主题通过 CSS 触发隐式网络请求，宿主会拒绝挂载包含 `@import` 或远程 `url(http/https/javascript)` 的 `cssText`。
