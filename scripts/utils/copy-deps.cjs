...
function copyPackages(targetNodeModulesDir) {
  const safeTargetNodeModulesDir = ensurePathWithinBase(
    PROJECT_ROOT,
  ...
  if (
    safeTargetNodeModulesDir === projectNodeModules ||
    isDescendantOf(safeTargetNodeModulesDir, projectNodeModules)
  ) {
    console.error(
      `[copy-deps] ERROR: target ${safeTargetNodeModulesDir} is inside the project's own node_modules, aborting`,
    );
    process.exit(1);
  }
...
function main() {
  ...
  const targetNodeModulesDir =
    path.basename(resolvedTarget) === "node_modules"
      ? resolvedTarget
